...
def handle_message(update: Update, context: CallbackContext):
    ...
    username = user.username or "(tanpa username)"
    first_name = user.first_name or "(tanpa nama)"

    # Debug log user info & pesan
    print(f"[DEBUG] Chat ID: {chat_id}")
    print(f"[DEBUG] Pesan dari @{username} ({first_name}): {user_message}")

    try:
        response = ask_sql(user_message)
        print(f"[DEBUG] Jawaban bot: {response}")
    except Exception as e:
        response = f"Maaf, terjadi error di handle_message(): {e}"
        print(f"[ERROR] {e}")
